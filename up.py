...
if st.button("🔮 Predict Range"):
    ...
    # Line + Bubble Chart: Energy Efficiency vs Speed
    speeds = list(range(20, 130, 10))
    consumption_rates = [energy_rate(s, terrain, weather) for s in speeds]
    bubble_data = pd.DataFrame({
        "Speed": speeds,
        "Rate": consumption_rates,
        "Size": [max(acceleration, braking)] * len(speeds),
    })
    ...
    fig_combo.add_trace(go.Scatter(
        ...
        name='Driving Impact',
        marker=dict(
            size=bubble_data["Size"] * 8,
            color='orange',
            opacity=0.6,
            line=dict(width=1, color='DarkSlateGrey')
        ),
        hoverinfo='text',
        text=[f"Speed: {s} km/h<br>Rate: {r:.2f} kWh/km" for s, r in zip(bubble_data["Speed"], bubble_data["Rate"])]
    ))

    fig_combo.update_layout(
        title="⚡ Energy Consumption vs Speed with Driving Impact",
        xaxis_title="Speed (Km/h)",
        yaxis_title="Energy Rate (kWh/km)",
        showlegend=True
    )

    vis_cols[2].plotly_chart(fig_combo, use_container_width=True)
